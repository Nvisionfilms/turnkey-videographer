...
export class ExportService {
  ...
  generateEmailText() {
    ...
    const originalTotal = Number(this.calc.originalTotal || this.calc.total || 0);
    const discountAmount = discountPercent > 0 ? Math.max(0, originalTotal - Number(this.calc.total || 0)) : 0;
...
LINE ITEMS:
${this.calc.lineItems.filter(item => !item.isSection).map(item => `- ${item.description}: $${(item.amount || 0).toFixed(2)}`).join('\n')}
...
${discountPercent > 0 && discountAmount > 0 ? `Discount (${discountPercent}%): -$${discountAmount.toFixed(2)}` : ''}
...
Generated by NVision Videographer Calculator
  ...
  generateClientHTML() {
    ...
    const useSimplifiedInvoice = this.settings?.simplified_invoice === true;

    const combinedLineItems = (() => {
      // If simplified invoice, group all production items into single line
      if (useSimplifiedInvoice) {
        const simplified = [];
        
        // Get ATL and BTL totals from calculations
        const atlTotal = this.calc?.atlTotal || 0;
        const btlTotal = this.calc?.btlTotal || 0;
        const productionCost = this.calc?.productionCost || (atlTotal + btlTotal);
        
        // Add single Production Cost line (combines all crew/services)
        if (productionCost > 0) {
          simplified.push({
            description: 'Production Cost',
            quantity: 1,
            unitPrice: productionCost,
            amount: productionCost
          });
        }
        
        // Add non-labor items separately (gear, travel, etc.)
        for (const item of rawLineItems) {
          if (item?.isSection) continue;
          const desc = (item?.description || '').toLowerCase();
          // Skip labor items (already in Production Cost)
          if (item?.category === 'atl' || item?.category === 'btl') continue;
          if (desc.includes('service fee') || desc.includes('overhead + margin')) continue;
          // Keep other items (gear, travel, tax, etc.)
          simplified.push({
    ...
    return `
      <div class="totals">
        ...
        ${this.formData.custom_discount_percent > 0 ? `
          <div class="total-row" style="color: #4C6FFF; font-weight: 600;">
            <span>Discount (${Number(this.formData.custom_discount_percent || 0)}%):</span>
            <span>-$${Math.max(0, Number(this.calc.originalTotal || 0) - Number(this.calc.total || 0)).toFixed(2)}</span>
          </div>
        ` : ''}
        <div class="total-row">
          <span>Tax:</span>
