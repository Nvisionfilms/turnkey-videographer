...
// Run affiliate commissions ledger migration
app.post('/api/admin/run-migration', async (req, res) => {
  ...
  try {
    ...
    // Create indexes
    await client.query(`CREATE INDEX IF NOT EXISTS idx_commissions_affiliate_status ON affiliate_commissions (affiliate_id, status)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_commissions_eligible_at ON affiliate_commissions (eligible_at)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_commissions_payment_intent ON affiliate_commissions (payment_intent_id)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_commissions_checkout_session ON affiliate_commissions (checkout_session_id)`);
    
    // Add refund tracking columns to affiliates table
    await client.query(`ALTER TABLE affiliates ADD COLUMN IF NOT EXISTS refund_count INT DEFAULT 0`);
    await client.query(`ALTER TABLE affiliates ADD COLUMN IF NOT EXISTS last_refund_at TIMESTAMPTZ`);
    
    await client.query('COMMIT');
    
    console.log('✅ Migration completed: affiliate_commissions_ledger');
    
    res.json({
      success: true,
      message: 'Migration completed successfully',
      tables: ['affiliate_payout_batches', 'affiliate_commissions'],
      columns: ['affiliates.refund_count', 'affiliates.last_refund_at']
    });
  } catch (error) {
    await client.query('ROLLBACK');
